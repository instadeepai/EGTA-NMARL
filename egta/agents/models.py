...
class IA2C:
    ...
    def crop_batch(self, x):
        return x[:self.n_step]

    def backward(self, Rends, dt, summary_writer=None, global_step=None):
        cur_lr = self.lr_scheduler.get(self.n_step)
        for i in range(self.n_agent):
            if self.trans_buffer[i].length() >= self.n_step:
                obs, nas, acts, dones, Rs, Advs = self.trans_buffer[i].sample_transition(Rends[i], dt)
                obs = self.crop_batch(obs)
                nas = self.crop_batch(nas)
                acts = self.crop_batch(acts)
                dones = self.crop_batch(dones)
                Rs = self.crop_batch(Rs)
                Advs = self.crop_batch(Advs)
    ...
    def is_param_for_agent(self, name, id):
        # NOTE override this per algorithm
        return name.startswith(f"lstm_{id}")

    def get_variables(self, agent_id=None):
        if agent_id is None:
            pred = lambda name: True
        else:
            pred = lambda name: self.is_param_for_agent(name, agent_id)

        # NOTE: this should be changed if we change optimizer
        return {
            v.name: v
            for v in self.sess.graph.get_collection(tf.GraphKeys.GLOBAL_VARIABLES)
            if "Adam" not in v.name and pred(v.name)
        }

    def get_variables_numpy(self, id=None):
        return self.sess.run(self.get_variables(id))

    def load_agent(self, other, my_id, other_id):
        new_vals = other.get_variables_numpy(other_id)
        my_vars = self.get_variables(my_id)

        self.sess.run(
            [
                v.assign(val)
                # let's hope they're in the right order
                for v, val in zip(my_vars.values(), new_vals.values())
            ]
        )
    ...
    def save(self, model_dir, global_step):
        self.saver.save(self.sess, model_dir + 'checkpoint-' + f"{global_step}")
    ...
    def _init_scheduler(self, model_config):
        ...
        if lr_decay == 'constant':
            ...
            warm_up_ratio = model_config.getfloat('warm_up_ratio')
            self.lr_scheduler = Scheduler(lr_init, lr_min, self.total_step, decay=lr_decay, warm_up_ratio=warm_up_ratio)
    ...
    def _init_train(self, model_config, distance_mask, coop_gamma):
        ...
        for i in range(self.n_agent):
            # init loss
            self.policy[i].prepare_loss(v_coef, e_coef, max_grad_norm, alpha, epsilon, optimizer)
            # init replay buffer
...
class IA2C_INDEPENDENT(IA2C):
    """
    Fully independent A2C learners, i.e. no centralised critic
    """
    def __init__(self, n_s_ls, n_a_ls, neighbor_mask, distance_mask, coop_gamma,
                 total_step, model_config, seed=0):
        self.name = 'ia2c_ind'
        n_agent = len(neighbor_mask)
        neighbor_mask = np.zeros((n_agent, n_agent), int) # disconnect all neighbours
        self._init_algo(n_s_ls, n_a_ls, neighbor_mask, distance_mask, coop_gamma,
                        total_step, seed, model_config)
...
class MA2C_NC(IA2C):
    ...
    def _init_train(self, model_config, distance_mask, coop_gamma):
        ...
        optimizer = model_config.get('optimizer')
        # init loss
        self.policy.prepare_loss(v_coef, e_coef, max_grad_norm, alpha, epsilon, optimizer)
        # init replay buffer
