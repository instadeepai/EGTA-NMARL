...
class OnPolicyBuffer(TransBuffer):
    ...
    def length(self):
        assert len(self.obs) == len(self.acts) == len(self.rs) == len(self.vs) == len(self.adds)# == len(self.dones)
        return len(self.obs)
...
"""
class Scheduler:
    def __init__(self, val_init, val_min=0, total_step=0, decay='linear', warm_up_ratio=0.1, min_ratio=0.1):
        self.val = val_init
        ...
        if decay == "exponential":
            orders_of_mag_drop = 2
            num_steps_until_min = total_step * (1 - warm_up_ratio - min_ratio)
            # val_min = val_init * 10**(-orders_of_mag_drop)

            # Warm up
            warm_up = int(warm_up_ratio * total_step)
            warm_up_trajectory = np.linspace(val_min, val_init, num=warm_up)

            # Exponential decay
            num_decay_steps = total_step * (1 - warm_up_ratio)
            exponents = np.arange(num_decay_steps) * orders_of_mag_drop / num_steps_until_min
            decay_trajectory = val_init / np.power(10, exponents)

            rates = np.append(warm_up_trajectory, decay_trajectory)

            # clamp learning rates to a minimum of min_rate
            self.rates = np.maximum(val_min, rates)
    ...
    def get(self, n_step):
        ...
        if self.decay == 'linear':
            ...
            return self.rates[min(self.n, len(self.rates) - 1)]
        else:
            return self.val
